...
import com.kaizzinho.battleslider.client.PokemonSpriteResolver
...
class BattleSliderConfigScreen(
    private val parent: Screen?
) : Screen(Text.translatable("battleslider.config.title")) {

    private enum class Page(val translationKey: String) {
        GENERAL("battleslider.config.tab.general"),
        ENCOUNTERS("battleslider.config.tab.encounters"),
        VISUALS("battleslider.config.tab.visuals"),
        PORTRAITS("battleslider.config.tab.portraits"),
        AUDIO("battleslider.config.tab.audio"),
        ADVANCED("battleslider.config.tab.advanced")
    }

    private var page = Page.GENERAL
    private var working = BattleSliderConfig.snapshot()
    ...
    private fun addVisualsPage() {
        ...
        addEnumOption(
            row++,
            "battleslider.config.animation_speed",
            BattleSliderConfig.AnimationSpeed.entries,
            BattleSliderConfig.AnimationSpeed.from(working.animationSpeed),
            { value ->
                Text.translatable(
                    "battleslider.config.value.animation_speed.${value.configValue}"
                )
            }
        ...
        addEnumOption(
            row++,
            "battleslider.config.flash_intensity",
            BattleSliderConfig.FlashIntensity.entries,
            BattleSliderConfig.FlashIntensity.from(working.flashIntensity),
            { value ->
                Text.translatable(
                    "battleslider.config.value.flash.${value.configValue}"
                )
            }
        ...
        addEnumOption(
            row++,
            "battleslider.config.hold_duration",
            holdValues,
        ...
        addEnumOption(
            row++,
            "battleslider.config.particles",
            BattleSliderConfig.ParticleDensity.entries,
            BattleSliderConfig.ParticleDensity.from(working.particleDensity),
            { value ->
                Text.translatable(
                    "battleslider.config.value.particles.${value.configValue}"
                )
            }
    ...
    private fun addPortraitsPage() {
        ...
        addEnumOption(
            row++,
            "battleslider.config.trainer_portraits",
            BattleSliderConfig.TrainerPortraitMode.entries,
            BattleSliderConfig.TrainerPortraitMode.from(
                working.trainerPortraitMode
            ),
            { value ->
                Text.translatable(
                    "battleslider.config.value.trainer_portrait.${value.configValue}"
                )
            }
        ...
        addEnumOption(
            row,
            "battleslider.config.pokemon_portraits",
            BattleSliderConfig.PokemonPortraitMode.entries,
            BattleSliderConfig.PokemonPortraitMode.from(
                working.pokemonPortraitMode
            ),
            { value ->
                Text.translatable(
                    "battleslider.config.value.pokemon_portrait.${value.configValue}"
                )
            }
    ...
    private fun addAudioPage() {
        ...
        addBooleanOption(
            row++,
            "battleslider.config.ui_sounds",
            working.uiTransitionSounds
        ) { working.uiTransitionSounds = it }

        addBooleanOption(
            row++,
            "battleslider.config.team_ball_sound",
        ...
        addEnumOption(
            row,
            "battleslider.config.team_ball_volume",
            volumes,
    ...
    private fun addAdvancedPage() {
        addBooleanOption(
            0,
            "battleslider.config.debug_logging",
            working.debugLogging
        ) { working.debugLogging = it }

        val reset = ButtonWidget.builder(
            Text.translatable("battleslider.config.reset_defaults")
        ) {
            ...
            working =
                BattleSliderConfig.defaultValues().copy(
    ...
    private fun addFooterButtons() {
        ...
        addDrawableChild(
            ButtonWidget.builder(
                Text.translatable("battleslider.config.save")
            ) {
                saveError =
                    !BattleSliderConfig.applyAndSave(working)
    ...
    override fun render(
        ...
        drawCentered(
            context,
            Text.translatable(
                "battleslider.config.page.${page.name.lowercase()}"
            ),
            panelTop + 23,
            0xFFB9B9C4.toInt(),
            false
        ...
        if (saveError) {
            drawCentered(
                context,
                Text.translatable(
                    "battleslider.config.save_failed"
                ),
    ...
    private fun renderSpritePackStatus(context: DrawContext) {
        ...
        val status = if (spriteAvailability.hasAnySprites) {
            Text.translatable(
                "battleslider.config.sprite_pack.detected",
                spriteAvailability.normalSpriteCount,
                spriteAvailability.shinySpriteCount
            )
        } else {
            Text.translatable(
                "battleslider.config.sprite_pack.missing"
            )
        }
        ...
        drawCentered(
            context,
            Text.translatable(
                "battleslider.config.sprite_pack.hint"
            ),
            y + 12,
            0xFF9A9AA6.toInt(),
            false
    ...
    private fun renderAdvancedInfo(context: DrawContext) {
        val overrideText =
            Text.translatable(
                "battleslider.config.rct_overrides",
        ...
        drawCentered(
            context,
            Text.translatable(
                "battleslider.config.rct_overrides_hint"
            ),
            rowY(1) + 18,
            0xFF808088.toInt(),
            false
