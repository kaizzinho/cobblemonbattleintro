...
object BattleIntroductionKeybinds {

    private val LOGGER =
        LoggerFactory.getLogger("battleintroduction/BattleIntroductionKeybinds")
    ...
    fun register() {
        if (registered) {
            LOGGER.warn(
                "BattleIntroductionKeybinds.register() ignored: already registered"
            )
            return
        }
        registered = true
        ...
        skipIntroKey = KeyBindingHelper.registerKeyBinding(
            KeyBinding(
                "key.battleintroduction.skip_intro",
                InputUtil.Type.KEYSYM,
                GLFW.GLFW_KEY_V,
                "category.battleintroduction"
            )
        )
